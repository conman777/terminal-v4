...
export async function updateThreadMetadata(
  ...
  session.thread = {
    ...currentThread,
    aiType: updates.aiType ?? currentThread.aiType ?? null,
...
export async function getThreadMetadata(
  ...
  return {
    ...
    ...(session.thread || {}),
    aiType: session.thread?.aiType ?? null,
    sandboxMode: session.thread?.sandboxMode ?? session.sandbox?.mode ?? 'off',
    sandboxWorkspaceRoot: session.thread?.sandboxWorkspaceRoot ?? session.sandbox?.workspaceRoot ?? null
  };
}
