...
export const TerminalPane = memo(function TerminalPane({
  ...
  const { refreshSessionGitStats } = useTerminalSession();
  ...
  const gitStats = currentSession?.thread?.gitStats || null;
  ...
  // Refresh git stats on session change and periodically (every 30s)
  useEffect(() => {
    if (!pane.sessionId) return;
    refreshSessionGitStats(pane.sessionId);
    const interval = setInterval(() => refreshSessionGitStats(pane.sessionId), 30_000);
    return () => clearInterval(interval);
  }, [pane.sessionId, refreshSessionGitStats]);
